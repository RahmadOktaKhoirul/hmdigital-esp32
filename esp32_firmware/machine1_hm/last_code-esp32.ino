...
const char* ssid        = "VAT BSC 11";
const char* password    = "22Nov2022";
...
void printBigHM(String text) {
  ...
  if (width <= 20) {
    ...
    String label = "-- HOUR METER --";
    String value = text + " HOURS";

    int colLabel = (20 - label.length()) / 2;
    int colValue = (20 - value.length()) / 2;

    lcd.setCursor(colLabel > 0 ? colLabel : 0, 1);
    lcd.print(label);

    lcd.setCursor(colValue > 0 ? colValue : 0, 2);
    lcd.print(value);
  }
}
...
void saveToNVS() {
  ...
  preferences.putULong("prev_sec",   prev_hm_seconds);
...
void publishData() {
  ...
  client.publish(topic_data, buf, true);
}
...
void setup() {
  ...
  prev_hm_seconds = preferences.getULong("prev_sec",  0);
...
void loop() {
  ...
  // 1. CEK BENDERA RESET DENGAN PROTEKSI GLITCH
  if (flagResetButton) {
    flagResetButton = false;
    // Proteksi: Abaikan penekanan tombol reset pada 3 detik pertama saat tegangan baru masuk
    if (millis() > 3000) { 
      resetHourMeter();
    } else {
      Serial.println("[WARN] Glitch tombol reset saat boot terdeteksi dan diabaikan!");
    }
  }
  ...
  // Save ke NVS saat engine berhenti (RUNNING → STOPPED)
  ...
  if (currentEngine && (currentMillis - previousMillis >= hm_interval_ms)) {
    ...
    // AUTO-SAVE SETIAP 60 DETIK (1 MENIT)
    if (hm_seconds - last_save_seconds >= 60) {
      saveToNVS();
      Serial.printf("[NVS] Saved periodic (1 Min): %.2fh\n", hm_seconds / 3600.0);
    }
